...
class InvalidFormatVersionError(ValueError):
    def __init__(self):
        super().__init__("FORMAT_VERSION must be exactly 3 bytes.")


class FfmpegNormalizeNotFoundError(FileNotFoundError):
    def __init__(self):
        super().__init__("ffmpeg-normalize not found in PATH")


class InvalidContainerFormatError(ValueError):
    def __init__(self):
        super().__init__("Invalid container format")


class InvalidUtf8MetadataError(ValueError):
    def __init__(self, raw_bytes: bytes):
        super().__init__(f"Invalid UTF-8 metadata: {raw_bytes!r}")


class UnrealisticMetadataLengthError(ValueError):
    def __init__(self, length: int):
        super().__init__(f"Unrealistic metadata length: {length}")
...
FORMAT_VERSION = FormatVersion(b"1.1")
...
def transcode_tracks(tracklist, temp_mgr):
    ...
    for i, track in enumerate(tracklist, start=1):
        ...
        if not Path(track).is_file():
            logging.warning(f"Skipping invalid track path: {track}")
            continue
        ...
        try:
            ...
            if STRICT_MODE:
                ...
                logging.warning(f"❌ ffmpeg failed for track {i}: {e}")
                continue
...
def concat_tracks(temp_files, pre_pcm):
    ...
    cmd = shutil.which("ffmpeg-normalize")
    subprocess.run([cmd, *temp_files, pre_pcm])
...
def normalize_pcm(pre_pcm, output_pcm):
    if USE_NORMALIZATION:
        ...
        cmd = shutil.which("ffmpeg-normalize")
        if cmd is None:
            raise FfmpegNormalizeNotFoundError()
        subprocess.run([
            cmd,
            pre_pcm,
            "-o",
...
def build_container(tracklist_file, output_file):
    ...
    created_str = f"Created by cdda2img v{VERSION} on {timestamp}"
...
def read_header(file):
    with open(file, "rb") as f:
        ...
        if magic != b"RBI":
            raise InvalidContainerFormatError()
        values = struct.unpack("<IIII", fixed[6:22])
        toc_start, toc_end, pcm_start, pcm_end = values
        ...
        if metadata_len > 1024:
            raise UnrealisticMetadataLengthError(metadata_len)

        created_by_bytes = f.read(metadata_len)
        try:
            created_by = created_by_bytes.decode("utf-8")
        except UnicodeDecodeError as err:
            raise InvalidUtf8MetadataError(created_by_bytes) from err
