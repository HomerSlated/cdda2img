...
15. **MB→Discogs link check on selected release**: §10.3.1 — follow the MB→Discogs link, compare barcodes (validates the relation, not the pressing), record `discogs_link_barcode_agrees` / `discogs_link_barcode_conflict`. PROV-only.
...
| MB→Discogs link conflict | the linked Discogs release's barcode disagrees with MB's | `discogs_link_barcode_conflict` recorded | Disc kept; conflict surfaced in PROV |
