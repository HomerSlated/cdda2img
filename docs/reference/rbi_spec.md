...
| `corroborated_release`     | An MB release UUID. Emitted only when the user changed the pressing in the alternatives menu **and** a corroboration key (`acoustid_corroborates`, `discogs_link_barcode_agrees`) is present: those checks run before the menu, against the release the ladder pinned, so after a manual change they describe a release the container no longer claims to be. Names the release they were actually taken against. Absent when the selection did not change or no corroboration ran. |
...
| `discogs_link_barcode_agrees` | `YES`. Emitted when the selected release's MB→Discogs url-relation resolved a Discogs release whose barcode **agreed** with MB's. This validates *the relation*, not the pressing: an MB editor supplied both the barcode and the link, so the two sides are not independent sources. It detects a mis-linked relation; it is **not** cross-source confirmation of which pressing the disc is. Absent on conflict or when the check did not run (no disc-ID match, no Discogs link/token, or no barcode on either side). PROV-only; does not feed selection or confidence. Named `discogs_barcode_corroborates` before v6.0.1 — containers written earlier carry the old key, which overstated the claim. |
| `discogs_link_barcode_conflict` | `mb:<barcode>\|discogs:<barcode>`. Emitted when that same url-relation resolved a Discogs release whose barcode **disagreed** with MB's — positive evidence that the relation points at the wrong release. Absent on agreement (see `discogs_link_barcode_agrees`) or when the check did not run. Named `discogs_barcode_conflict` before v6.0.1. |
