...
| `discogs_barcode_corroborates` | `YES`. Emitted when the selected release's MB→Discogs url-relation resolved a Discogs release whose barcode **agreed** with MB's (cross-source confirmation). Absent on conflict or when no Discogs corroboration ran (no disc-ID match, no Discogs link/token, or no barcode on either side). PROV-only; does not feed selection or confidence. |
| `discogs_barcode_conflict` | `mb:<barcode>\|discogs:<barcode>`. Emitted when that same url-relation resolved a Discogs release whose barcode **disagreed** with MB's. Absent on agreement (see `discogs_barcode_corroborates`) or when no Discogs corroboration ran. |
