...
- **`tools/accudisc/`** — snapshot of the AccuDisc CLI (`accudisc`) + its Plextor driver
  (`drivers/accudisc-drv-plextor.so`), the binary `accudisc_reader.py` drives. Re-snapshot
  from the accudisc build when its interface changes (see `accudisc_reader.py` header).
...
- **`accudisc_reader.py`** — the C2 read path: AccuDisc subcommand wrappers (`read_disc_c2`/`read_span`/`drive_supports_c2`/`probe_combos`/`park_spindle`), same interface as `c2_reader.py` which it replaced. Resolves the `tools/accudisc/accudisc` snapshot (falls back to `$PATH`). Absorbs the flag→subcommand deltas: `--c2f` (not `--c2`), whole-disc = `read` with no `--count`, `cdtext`/`fulltoc` as separate lead-in subcommands, progress parsed from the `\r`-updated stderr sector line, exit `!= 0` fatal (no c2read `3`), `cdtext` non-zero = benign no-CD-Text. `c2_reader.py` (c2read) is kept as reference/fallback.
