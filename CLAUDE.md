...
1. `cdda2img.py:_rip_disc_stage()` — chooses the read path. **Normal** (`c2_recovery=off`, default): `_rip_with_fallback()` — `cdrdao_ripper.rip_cdrdao()` (primary) → `disc_reader.rip_disc(paranoia="full")` on RuntimeError. **C2 path** (`c2_recovery=on`, or `auto` + `c2_reader.drive_supports_c2()`): `c2_reader.read_disc_c2()` (raw s16le audio + C2 bitmap via `c2read --full`) plus a `cdrdao_ripper.read_toc_metadata()` pass (`cdrdao read-toc` for pre-gaps/ISRC/MCN/CD-Text — a second read, since c2read can't read the subchannel yet; also sidesteps bug #75). cdrdao/c2read reads return **raw** PCM; only a cd-paranoia read fallback is offset-corrected.
