...
- **`test`** — verify all block checksums and structural invariants (the spec's 33 validation rules, incl. §6.2.1 PCM-vs-TOC geometry; exits 1 on failure)
...
- **`tools/recovery_bench.py`** — **the last CLI consumer in the tree** (2026-08-01). It
  ...
  the instrument behind the seven bench-arm profiles and RECOVERY.md's numbers, so a
...
1. `cdda2img.py:_rip_disc_stage()` — **one engine, one pass.** A single `accudisc_reader.read_disc_c2()` call captures raw s16le audio + the C2 bitmap + raw P-W subchannel, plus the full-TOC and CD-Text lead-in dumps on the **same `Device`** (one spin-up); real TUI progress comes from the read sink's own chunk callback. `subq_toc.build_rip_info()` then assembles the disc metadata from those captures (rip_type `accudisc`) — pre-gaps/INDEX/CONTROL from the Q stream, majority-voted MCN/ISRC (structurally immune to cdrdao bug #75), track starts from the error-corrected full TOC (Q never moves a boundary). Track 1's pre-gap is derived from the full TOC, not the Q stream: when track 1's INDEX 01 is at LBA > 0 (a program-area pre-gap that the `[0, lead-out)` read captured — e.g. ABBA *Gold*'s 33 frames), those frames are declared as track 1's pre-gap (`start_frame=0`, `pregap_frames=start`); dropping them shifted every boundary and the lead-out down and broke the round-trip disc ID (fixed 2026-07-12). Validated at full field parity against `cdrdao read-toc` (`tools/toc_parity.py`). **There is no second metadata pass and no second engine** — if assembly fails, the rip fails (`subq_toc` already degrades to TOC-only geometry when the Q stream can't be anchored, so reaching that point means the full TOC itself is unusable). `cfg.c2_recovery` **defaults to `auto` (flipped from `off` 2026-07-29, user-authorised 2026-07-05)** and controls exactly one thing: whether the C2 bitmap is *written* to a scratch file. C2 pointers are requested on the wire on every read regardless — the seam sets `ACCUDISC_C2_PTRS` unconditionally — so the sector is 2646 B either way and the read costs the same. `off` therefore bought ~48 MB of scratch and gave up the erasure boost that roughly doubles what ctanalyse can reconstruct. **That argument is about the read; the flip also changes the repair** — with a bitmap present, CTDB recovery now runs **C2-erasure-assisted first, error-only as fallback** by default, where it previously went straight to error-only. Those are genuine alternatives (an over-flagging bitmap can spend erasure budget on clean words and turn a decodable stride undecodable), and both orders are covered by `tests/test_ctdb_repair.py` §D1 — but the default now selects the first. `off` remains honoured as an escape hatch. AccuDisc zero-fills hard-unreadable sectors (PCM zeros + C2 all-ones = pure erasures downstream) and returns **raw** PCM — `apply_offset` runs exactly once, at storage. (AccuDisc is a separate external project (https://github.com/HomerSlated/accudisc) — not shipped here; its Python binding is reached via the git-ignored `tools/accudisc/pybinding` symlink in a checkout, or an injected wheel in an install.)
   ...
   - **Flagged-span rung (opt-in, between the two exits)**: only with `--profile span-flagged` (`granularity = "span"`), `_recover_flagged_spans` re-reads just the capture pass's flagged sectors in padded spans, overlays accepted copies in memory and writes a track only on an AccurateRip match; the ladder below then gets only the tracks it did not recover. Never run on hardware (kgr, 2026-09-24). A **total** AR miss runs neither exit, by design, and says so (`_warn_ar_total_miss`) before the offset search.
...
- **`src/cdda2img/profiles/*.toml`** — eight shipped profiles. `span-flagged` is the flagged-span rung (`_recover_flagged_spans`, `docs/reference/span-recovery-plan.md`): **opt-in, experimental, and never run on hardware** (kgr cancelled its bench runs on 2026-09-24). It runs after CTDB and before the ladder, and hands the ladder only the tracks it did not recover. The other seven are one per bench arm: `track-ladder` (default, 19/20), `track-constant` (14/19), `max-variation` (13/20, fast at low n and 0/9 at high n), `whole-disc` (11/20), and the experimental `sector-runup` / `sector-hammer` / `span-fixed` (2/20, 2/20, 1/16). The experimental three are kept as **controls**, not candidates: `sector-hammer` anchors the zero point of the variation axis and `span-fixed` is what separates span size from variation as the active ingredient.
...
- **AccurateRip transport (R2)**: HTTPS is preferred; HTTP fallback only on `URLError` / `OSError`. A 404 over HTTPS is a legitimate negative ("disc not in DB") and does *not* fall back to HTTP. Responses are capped at `_AR_DBAR_MAX = 1 MB`. Per-block `(id1, id2, cddb_id)` header verification drops mismatching blocks at WARNING level — protects against a poisoned plaintext response splicing unrelated discs' blocks. `verify_rip` returns `ARVerifyResult(tracks, transport, dbar_b3sum)`. **Unreachable is not "not in database" (2026-09-10).** An AccurateRip that never answered yields the same all-`None` per-track results as a disc it has never heard of, and until then all three records said "not in database": the printed report, the ARIP block (status `0 NOT_IN_DB`, sealed), and the RLOG (sealed). `ARVerifyResult.reachable` (`transport is not None`) now carries the distinction: `format_ar_report(..., reachable=)` says *not verified*, the ARIP block is **omitted** rather than filled with false status-0 claims (§6.5 — no format bump, the block is optional), the RLOG writes `Not verified (AccurateRip unreachable)`, and PROV gains `lookup_status_accuraterip=OK|empty|down`. A **5xx no longer records a transport** — the code did until then, contradicting its own dataclass comment and the spec's `arip_transport` row. **Skipping CTDB repair and the re-read ladder when AR is unreachable is correct and unchanged**: both are keyed on an AR mismatch and need AR to re-verify what they splice. The defect was only that the skip was silent; `rip_image` now prints why (`_warn_ar_unreachable`). **Auditing older containers:** before the fix, `arip_transport` was written only when a server answered, so *absent `arip_transport` + ARIP status 0* means AR was unreachable at the network level, while *present + status 0* is a 404 **or** a 5xx and cannot be told apart.
...
- **Network pre-flight (2026-09-10): an archiver without provenance is functionally useless, so ask before starting, not after.** `network_preflight.py` probes every service the pipeline will use **in parallel on daemon threads under one wall-clock deadline** (`DEADLINE = 8` s) — the deadline, not the per-probe timeout, is what bounds DNS, which neither socket timeout mechanism above can. Probes never follow redirects (MB `/ws/2/` answers 301); any 2xx/3xx/4xx is an answer, 5xx is down, **MB 503 is "busy"** (its rate limiter), CDDB is judged by its `20x` greeting. Required services: `rip` = AccurateRip + MusicBrainz, `create`/`import` = MusicBrainz; the rest are advisory. Policy: a TTY without `--auto` gets retry/continue/abort (empty input aborts); headless **refuses (exit 1) unless `--allow-offline`**. The outcome is recorded as PROV `network_preflight=ok|degraded:<svcs>`. Skipped for `import --info` and `rip --list-profiles`. **Mid-run failures (2026-09-13)**: a required service that fails after the pre-flight is offered for retry *where it failed*, on a TTY without `--auto` (`_interactive`): `_verify_ar_with_retry` wraps the rip's **first** AR verification (the later re-verifications only run after it answered), and `_retry_mb_lookup` repeats only the disc-ID lookup inside `_run_metadata_lookups` (CDDB already answered or failed on its own; a failed lookup left `disc` untouched). `_offer_service_retry` is `[R/c]`, Enter retries, Ctrl-D continues, and there is **no abort**: the disc has been read by then, and continuing costs nothing a re-run would not. **Exit status 3**: `rip_image`/`import_image` return `_unanswered_services(provenance)` (a `lookup_status_accuraterip`/`lookup_status_mb` of `down`), `_dispatch` returns an rc through `_report_unanswered`, and `main` raises `SystemExit(rc)`. `create` never exits 3: it performs no disc-ID lookup (`lookup_status_mb=disabled`) and no AccurateRip check, so nothing in its PROV can be `down`.
- **Exit status 4: written, but a recorded verification failed (2026-09-14).** `rip_image` returns `RipOutcome(unanswered, failed_checks)`; `_failed_checks(provenance)` fires on `ar_total_miss` (AccurateRip holds the disc, no track verified) or `subq_q_valid` below `_MIN_Q_VALID_FRACTION = 0.075` (misframed reads measure 0% or 1/chunk; the lowest with good audio is 13.43%, a contended drive), and `_report_rip_outcome` exits 4, which beats 3. **Neither refuses the rip**: both happen with good audio (a pressing AR does not hold; a drive returning sub before C2 at a correct stride zeroes Q exactly as misframing does, measured 0/23 both ways), so the container is kept for inspection. The trigger was a LITE-ON LH-20A1S rip misframed by **libata's mod-16 rule** (an ATAPI buffer not a multiple of 16 bytes goes by PIO, and that drive pads each record to 16 B: 23 × 2742 drifted 10 B per sector). It verified 0/11 and exited 0 with `Health status: No errors occurred`, a line `rip_log.py` wrote unconditionally and no longer writes (rbi_spec §6.6.1). **Three reporting defects went with it.** `_diagnose_total_ar_miss` now keeps only `confirmed` candidates (`detect_offset` always appends 0, so an unfiltered list reported `offset_mismatch` on nothing). `format_ar_report` no longer tells the user to add a `[[drives]]` entry: nothing in the per-track results supports it, and on that rip the entry existed and the offset was right. Do not read a 0/N as an offset problem before checking `subq_q_valid`.
- **MusicBrainz "could not ask" is not "not known" (2026-09-13).** `mb_lookup.lookup_disc_id` returns `[]` **only** for a 404 (cached); every other failure raises `MBLookupError` with `reason` = the HTTP status or `network` (never cached). Until then both returned `[]` and `_r12_status(errored=False)` was hard-coded, so `lookup_status_mb` could never be `down`. `prepopulate_from_mb` catches it, logs a WARNING and returns `MBPrepopResult(lookup_error=reason)`; `_run_metadata_lookups` records `lookup_status_mb=down` + `lookup_error_mb=<reason>`, and **skips the R4 ISRC tally and stage 7**, both of which ask the same service and would retry for minutes to reach the same failure. A 4xx other than 404 is still `down` (the vocabulary stays four values) but `lookup_error_mb` keeps a request defect (`400`) apart from an outage (`503`, `network`). The banner callers treat the error as "(unknown)". **Retry notice**: musicbrainzngs retries a stalled request 8 times inside the private `_safe_read(max_retries=8)` with no public setter, so the delay cannot be shortened without patching the library, and is not. **The delay has two sizes**: 56 s when MusicBrainz answers every attempt with an error (measured 2026-09-13, real library against a local 503 server — sleeps only, 2+4+…+14 s), and ~5 min when the connection stalls (8 × 30 s socket timeout plus those sleeps). `mb_lookup.RetryNotice`, installed on the `musicbrainzngs` logger by `_install_log_handler`, turns its INFO retry records into `MusicBrainz slow|busy: try N/8` — 7 notices for 8 attempts, `2/8`..`8/8`, since the first attempt logs no retry (≤ 26 chars, the TUI's status budget at 80 columns) via `TerminalUI.set_status_text`, or a line of its own without a TUI. Lowering that logger to INFO is why the terminal handler now carries its own WARNING level at default verbosity: propagation ignores the root logger's level. `tests/test_mb_retry_notice.py` pins the library's retry count and log strings as controls.
