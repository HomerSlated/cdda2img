...
- **`cdtext.py`** — CD-Text pack decoder for raw READ TOC format-0x05 dumps: 18/16-byte stride detection, per-pack CRC (reuses `crc16_gsm`), NUL-separated string reassembly, TAB shorthand, SIZE_INFO; Latin-1/ASCII block 0 only (MS-JIS skipped)
- **`subq_toc.py`** — `build_rip_info(fulltoc_raw, sub_data, cdtext_raw)` — the F7 join point: assembles a `RipInfo` (with `prov` keys `toc_source=subq@c2read`, `subq_frames`, `isrc_vote_track_<n>`) from one c2read pass; TOC authoritative for boundaries, Q supplies pre-gaps/flags/MCN/ISRC, CD-Text supplies titles; degrades to TOC-only geometry when the Q stream can't be anchored
