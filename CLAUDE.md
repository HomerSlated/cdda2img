...
- **`tools/toc_parity.py`** — field-by-field parity gate: `cdrdao read-toc` vs the AccuDisc
...
- **AccuDisc** — the low-level CD-DA read engine `accudisc_reader.py` drives, a **separate
  ...
  **not shipped from this repo**. A local snapshot lives in `tools/accudisc/` (git-ignored,
  for testing/production only); the subprocess contract is frozen in the AccuDisc repo's
  `docs/cli-machine-interface.md`. Re-snapshot from an accudisc build when its interface
  changes (see the `accudisc_reader.py` header).
...
- **`tools/c2read_recovery_test.py`** — the c2read arm of the multi-pass recovery
  experiment (sequential sweeps / `--characterize` rate-per-speed / `--retries`
  attempts-to-match; AR the only gate). Its 6/6 result on the damaged reference disc is
  what justified replacing cd-paranoia in `_recover_failed_tracks` (2026-07-05).
...
1. `cdda2img.py:_rip_disc_stage()` — chooses the read path. **Normal** (`c2_recovery=off`, default): `_rip_with_fallback()` — `cdrdao_ripper.rip_cdrdao()` (primary) → `disc_reader.rip_disc(paranoia="full")` on RuntimeError. **C2 path** (`c2_recovery=on`, or `auto` + `accudisc_reader.drive_supports_c2()`): **one** `accudisc_reader.read_disc_c2()` pass (raw s16le audio + C2 bitmap + raw P-W subchannel via `accudisc read --sub raw`, plus inline `--fulltoc`/`--cdtext` lead-in dumps in the same pass; real TUI progress from AccuDisc's `--progress-fd 1` machine tokens (`progress <done> <total>` on stdout, `-q` muting the human line)), then `subq_toc.build_rip_info()` assembles the disc metadata from the captures (rip_type `accudisc`) — pre-gaps/INDEX/CONTROL from the Q stream, majority-voted MCN/ISRC (structurally immune to bug #75), track starts from the error-corrected full TOC (Q never moves a boundary). Track 1's pre-gap is derived from the full TOC, not the Q stream: when track 1's INDEX 01 is at LBA > 0 (a program-area pre-gap that the `[0, lead-out)` read captured — e.g. ABBA *Gold*'s 33 frames), those frames are declared as track 1's pre-gap (`start_frame=0`, `pregap_frames=start`); dropping them shifted every boundary and the lead-out down and broke the round-trip disc ID (fixed 2026-07-12). Validated at full field parity against `cdrdao read-toc` (`tools/toc_parity.py`). Only if assembly fails does the old `cdrdao_ripper.read_toc_metadata()` second pass run (rip_type `accudisc+toc`). AccuDisc zero-fills hard-unreadable sectors (PCM zeros + C2 all-ones = pure erasures downstream). cdrdao/AccuDisc reads return **raw** PCM; only a cd-paranoia read fallback is offset-corrected. (AccuDisc is the successor to the c2read prototype and a separate external project (https://github.com/HomerSlated/accudisc) — not shipped here; a git-ignored local snapshot in `tools/accudisc/` is used for testing/production. Flag→subcommand deltas are absorbed in `accudisc_reader.py`.)
   ...
   - **CTDB recovery (first exit)**: on a partial AR mismatch, `rip_image` first tries `ctdb_repair.repair_whole_disc()` — error-only ctanalyse (or C2-erasure-assisted when the C2 path captured a bitmap) on the raw PCM, committed only if a CTDB per-track-CRC **and** AccurateRip double-gate both pass; **zero extra reads**. Only if that fails (not in CTDB / over RS capacity / a gate rejects it) does the AccuDisc speed-ladder below run — the two are alternative exits in cost order. **Unified offset domain**: cdrdao/AccuDisc reads stay raw through AR + ctanalyse + the recovery ladder (verify at `read_offset`); `apply_offset` runs exactly once, at storage. PROV: `recovery_track_<n>=ctdb_repaired@<entry_id>`. Modules: `ctdb_repair.py` (canonical logic, reuses `accuraterip`), `accudisc_reader.py` (AccuDisc subcommand wrappers; `accudisc` is snapshotted in `tools/accudisc/`, `ctanalyse` is on `$PATH`). `c2_reader.py` remains as the c2read wrapper (reference/fallback). `tools/ctdb_repair.py` is the standalone CLI equivalent.
   - **AR-recovery ladder (rip only, unconditional fallback)**: after the rip, `rip_image` runs `accuraterip.verify_rip`; on a *partial* mismatch (some tracks are in the AR DB but unmatched), `cdda2img._recover_failed_tracks` re-reads each failed track's raw sector window via `accudisc_reader.read_span` across the drive's probed speed ladder (`drive_speed.probe_speed_ladder`, fastest→slowest, `cfg.recovery_passes` sweeps — total attempts = passes × ladder steps), AR-verifying each attempt's offset-corrected slice with `accuraterip.match_track_pcm` against responses fetched once (`fetch_ar_responses`) and splicing the **first match's verified corrected bytes** at `track_start*2352 + read_offset*4` into the still-raw PCM (sample-exact — neighbouring tracks are never perturbed); a track that never matches keeps its original audio (no unverified splice). rip_type gains `+c2rec`. The drive is restored to max **once** after the loop; eject/reset are **not** used (no measured recovery benefit). The sweep across passes × speeds is the recovery mechanism — validated 6/6 on the damaged reference disc (`tools/c2read_recovery_test.py`, `tools/paranoia_recovery_test.py` for the retired cd-paranoia baseline). cd-paranoia no longer plays any recovery role (it remains only as the full-disc *read* fallback when cdrdao fails). Outcomes recorded in PROV: `recovery_track_<n>=matched@<speed>X` | `unrecovered`, plus `recovery_passes` / `recovery_ladder`. `recovery_passes=0` disables recovery.
...
- **`accudisc_reader.py`** — the C2 read path: subprocess wrappers around the external **AccuDisc** engine (https://github.com/HomerSlated/accudisc; `read_disc_c2`/`read_span`/`drive_supports_c2`/`probe_combos`/`park_spindle`), same interface as `c2_reader.py` which it replaced. Resolves the git-ignored `tools/accudisc/accudisc` local snapshot (falls back to `$PATH`). Absorbs the deltas: `--c2f` (not `--c2`), whole-disc = `read` with no `--count`, inline `--cdtext`/`--fulltoc` lead-in capture (single spin-up), progress via `--progress-fd 1` machine tokens on stdout, exit `in (0, 3)` = completed (0 clean / 3 caveats), `1`/`2` fatal. The frozen subprocess contract is the AccuDisc repo's `docs/cli-machine-interface.md`. `c2_reader.py` (c2read) is kept as reference/fallback.
...
- The fallback uses `paranoia="full"`. This *read* fallback is cd-paranoia's only
  remaining role: AR-failure recovery is the AccuDisc speed-ladder
  (`_recover_failed_tracks`, via `accudisc_reader.read_span`), not cd-paranoia —
  validated 6/6 on the damaged reference disc; the sweep across passes × speeds is
  the recovery mechanism, not the paranoia engine (`tools/c2read_recovery_test.py`,
  2026-07-05).
