...
- **`accudisc_reader.py`** — **the** disc read path (there is no other): subprocess wrappers around the external **AccuDisc** engine (https://github.com/HomerSlated/accudisc; `read_disc_c2`/`read_span`/`read_toc`/`read_lead_in`/`drive_supports_c2`/`probe_combos`/`park_spindle`). Resolves the git-ignored `tools/accudisc/accudisc` local snapshot (falls back to `$PATH`). Absorbs the deltas: `--c2f` (not `--c2`), whole-disc = `read` with no `--count`, inline `--cdtext`/`--fulltoc` lead-in capture (single spin-up), progress via `--progress-fd 1` machine tokens on stdout, exit `in (0, 3)` = completed (0 clean / 3 caveats), `1`/`2` fatal. The frozen subprocess contract is the AccuDisc repo's `docs/cli-machine-interface.md`.
...
- **Subprocess**: `accudisc_reader.py`, `drive_speed.py`, `disc_writer.py`, `rip_log.py`, `write_offset.py`, `ctdb_repair.py` and `track_preview.py` spawn **`accudisc`** (the only disc engine — no cdrdao or cd-paranoia binary is invoked anywhere in `src/`); `audition.py` and `track_preview.py` spawn `ffplay`; intentional subprocess calls carry `# noqa: S603, S607` (see LINT-008, LINT-012, LINT-013, LINT-017)
